#!/bin/bash
# Submit all Qwen2-VL-2B training strategies as separate SLURM jobs
# Model: Qwen/Qwen2-VL-2B-Instruct
# WandB project: 23ComprehensivePipeline
# Account: master, Time: 71h
#
# Usage:
#   ./submit_qwen2_vl_2B_jobs.sh                    # Submit all jobs
#   ./submit_qwen2_vl_2B_jobs.sh --dry-run          # Show what would be submitted without actually submitting
#   ./submit_qwen2_vl_2B_jobs.sh --limit 5          # Only submit first 5 jobs (for testing)
...
# Qwen2-VL-2B config files (12 total)
CONFIG_FILES=(
    # Full FT Benchmark (3)
    "$CONFIG_DIR/conf_qwen2_vl_2B_full_sft_ocrbench.yaml"
    "$CONFIG_DIR/conf_qwen2_vl_2B_full_sft_docvqa.yaml"
    "$CONFIG_DIR/conf_qwen2_vl_2B_full_sft_chartqa.yaml"
    # Full FT ERP (6)
    "$CONFIG_DIR/conf_qwen2_vl_2B_full_sft_chosen_nova.yaml"
    "$CONFIG_DIR/conf_qwen2_vl_2B_full_sft_chosen_gemini.yaml"
    "$CONFIG_DIR/conf_qwen2_vl_2B_full_sft_qcm_nova.yaml"
    "$CONFIG_DIR/conf_qwen2_vl_2B_full_sft_qcm_gemini.yaml"
    "$CONFIG_DIR/conf_qwen2_vl_2B_full_sft_qcm_procedure1.yaml"
    "$CONFIG_DIR/conf_qwen2_vl_2B_full_sft_qcm_procedure2.yaml"
    # QLoRA Benchmark (3)
    "$CONFIG_DIR/conf_qwen2_vl_2B_qlora_sft_ocrbench.yaml"
    "$CONFIG_DIR/conf_qwen2_vl_2B_qlora_sft_docvqa.yaml"
    "$CONFIG_DIR/conf_qwen2_vl_2B_qlora_sft_chartqa.yaml"
)
...
if [ ${#VALID_FILES[@]} -eq 0 ]; then
    echo "Error: No Qwen2-VL-2B config files found"
    exit 1
fi

echo "=========================================="
echo "Qwen2-VL-2B Training Job Submission"
echo "=========================================="
